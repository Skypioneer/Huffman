...
// Date: 11/14/2020
// Purpose: Implement of the Class HuffmanTree
...
bool HuffmanTree::createCodeTree(string filename) {
    ...
    const int SIZE = 128;       // size of int freq array
    int freq[SIZE] = {0};             // create int array

    // populate contents from allText to freq array
    for(char c : allText) {
        freq[(int) c]++;
    }
...
bool HuffmanTree::encodeFile(string originalFilename, string outputFilename) {
    ...
    for (int i = 0; i < (int)allText.length(); i++) {
        ...
        vector<int> encodeString;       // create int vector
...
int HuffmanTree::traverseAndPrint(string &bits, int i, BSTNode *cur) {
    ...
    // recursive function to print out characters and its encoding code
